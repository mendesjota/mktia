...
def display_example_images(sidebar_example_dir: Path):
    ...
    if not sidebar_example_dir.exists():
        ...
        image_paths = list(sidebar_example_dir.glob("*.jpg")) + list(sidebar_example_dir.glob("*.jpeg"))
        if not image_paths:
            ...
            for idx, image_path in enumerate(image_paths):
                try:
                    ...
                    st.sidebar.error(f"Erro inesperado ao carregar '{image_path.name}': {e}")
    ...
    try:
        ...
        for m in list_models_response:
            ...
            if "generateContent" in m.supported_generation_methods:
                if "vision" in m.name.lower() or "flash" in m.name.lower() or "1.5-pro" in m.name.lower(): # Modelos de visão podem incluir 1.5-pro
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            if "image" in message and message["image"] is not None:
                ...
                elif isinstance(message["image"], (str, Path)): # Permite exibir caminhos de arquivo (para exemplos)
